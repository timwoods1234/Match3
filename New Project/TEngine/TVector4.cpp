...
//-------------------------------------------------------------------------------
TVector4 GetRandomVec4(unsigned int* pSeed, const TVector4& min, const TVector4& max)
{
	return TVector4(
		FastRandom(pSeed, min.x, max.x), 
		FastRandom(pSeed, min.y, max.y),
		FastRandom(pSeed, min.z, max.z),
		FastRandom(pSeed, min.w, max.w));
}

//-------------------------------------------------------------------------------
TVector4 GetRandomUnitVec4(unsigned int* pSeed)
{
	return Normalize( TVector4(
			FastRandom(pSeed, -1.0f, 1.0f), 
			FastRandom(pSeed, -1.0f, 1.0f),
			FastRandom(pSeed, -1.0f, 1.0f),
			FastRandom(pSeed, -1.0f, 1.0f) ));
}
